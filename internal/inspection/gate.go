...
const (
	// DirectionIn and DirectionOut are the two gate movements.
	DirectionIn  = "in"
	DirectionOut = "out"
)
...
// Inspect evaluates one gate pass. Holds are appended to the container's
// hold list and the pass is rejected while any hold exists.
...
// Release clears holds once the operator resolves them and confirms the
// current seal number.
...
// HoldsFor returns a copy of the container's current holds.
func (g *Inspector) HoldsFor(containerID string) []string {
	out := make([]string, 0, len(g.holds[containerID]))
	out = append(out, g.holds[containerID]...)
	return out
}
